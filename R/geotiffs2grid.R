...
dataset<-myconfig$ACTIVE_DATASET
cfg <- config::get(file = here::here("R", "config.yml"),config = dataset)
####========================================= ####


# run the script for each directory of geotiffs

DNB_geotiffs_DIR <- cfg$DNB_geotiffs_DIR1 #run again for cfg$DNB_geotiffs_DIR2



# Set directories variables -----------------------------------------------
geotiffs_DIR <- here::here("data",cfg$geotiffs_dir, DNB_geotiffs_DIR)
...
dnb_files <- list.files(geotiffs_DIR , pattern = sprintf("^VNP46A1.A%s.*\\.DNB*.*tif$",cfg$YEAR), full.names = T) #lunar corrected files (roman or cao method)
...
writeRaster(
    original_daily_DNB,
    here::here("data",cfg$grd_dir, DNB_geotiffs_DIR, cfg$original_DNB_grd),
    format = "raster",
    overwrite = TRUE,
...
writeRaster(
    daily_DNB,
    here::here("data",cfg$grd_dir, DNB_geotiffs_DIR, cfg$DNB_grd),
    format = "raster",
    overwrite = TRUE,
    datatype = 'INT2S' # χάνουμε λίγη πληροφορία με τους ακέραιους
) #read it with stack()
